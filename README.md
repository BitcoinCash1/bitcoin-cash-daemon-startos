...
**Block templates are always filled by fee.** The package starts BCHD with `--blockprioritysize=0`, so `getblocktemplate` skips BCHD's "high-priority" phase and includes transactions by fee rate from the first byte. Without it, the priority phase plus a 0-byte minimum block size can hand a mining pool a near-empty template while paying transactions wait.
...
9. **Block templates skip BCHD's priority phase** (`--blockprioritysize=0`), so they are ordered by fee rate throughout.
