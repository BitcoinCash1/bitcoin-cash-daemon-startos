...
export const setInterfaces = sdk.setupInterfaces(async ({ effects }) => {
  ...
  const rpc = sdk.createInterface(effects, {
    ...
    id: rpcInterfaceId,
    description:
      'JSON-RPC over TLS — the standard Bitcoin Cash RPC interface that most software uses. Compatible with the BCHN RPC subset that BCHD implements. Uses a BCHD self-signed certificate (generated by `gencerts`s. Compatible with the BCHN RPC subset that BCHD implements. Uses a BCHD self-signed certificate (generated by `gencerts`); clients must trust /data/rpc.cert or skip TLS verification.',
    type: 'api',
    masked: false,
  ...
  if (grpcEnabled) {
    ...
    const grpc = sdk.createInterface(effects, {
      ...
      id: grpcInterfaceId,
      description:
        'BCHD-specific gRPC API over TLS (BIP 157/158 compact block filters, pub/sub notifications). Rarely used in practice: client software has to be explicitly designed against BCHD\'s gRPC schema, and almost none is. Most users should ignore this and use the RPC Interface above. Uses a BCHD self-signed certificate (generated by `gencerts`e: client software has to be explicitly designed against BCHD\'s gRPC schema, and almost none is. Most users should ignore this and use the RPC Interface above. Uses a BCHD self-signed certificate (generated by `gencerts`); clients must trust /data/rpc.cert or skip TLS verification.',
      type: 'api',
      masked: false,
      schemeOverride: { ssl: 'https', noSsl: 'https' },
      username: null,
  ...
  const rpcPlaintext = sdk.createInterface(effects, {
    ...
    description:
      'Plaintext JSON-RPC for ckpool-lineage miners (asicseer-pool, ckpool) that have no TLS support. A bundled stunnel sidecar accepts plain HTTP on 8334 and forwards to BCHD\'s TLS RPC on 8332 internally — TLS is never absent on the BCHD side. Use the regular RPC Interface for anything elseto BCHD\'s TLS RPC on 8332 internally — TLS is never absent on the BCHD side. Use the regular RPC Interface for anything else.',
    type: 'api',
    masked: false,
