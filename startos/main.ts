...
export const main = sdk.setupMain(async ({ effects }) => {
  ...
  const bchdArgs: string[] = [
    ...
    // Fill block templates by fee from the first byte (no priority phase), so
    // low-fee and deep unconfirmed-chain transactions are not left out of
    // getblocktemplate. BCHD's default priority phase plus a 0-byte
    // blockminsize can otherwise produce near-empty templates on a backlog.
    `--blockprioritysize=0`,
  ]
